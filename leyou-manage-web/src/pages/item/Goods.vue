    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon @click="updateSaleable(props.item)" v-if="props.item.saleable">下架</v-btn>
          <v-btn icon @click="updateSaleable(props.item)" v-else>上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
  ...
  export default {
    ...
    methods: {
      ...
      async editGoods(oldGoods) {
        ...
        oldGoods.skus = await this.$http.loadData("/item/spu/list?id=" + oldGoods.id);
        // 修改标记
        this.isEdit = true;
        // 控制弹窗可见：
        this.show = true;
      ...
      deleteBrand(oldGoods){
        this.$http.get("/item/spu/delect/sid/" + oldGoods.id)
          .then(({data}) => {
            this.getDataFromServer();
      ...
      updateSaleable(oldGoods){
        this.$http.get("/item/spu/updateSaleable", {
          params:{
            sid:oldGoods.id,
            sFlag: oldGoods.saleable == 0 ? true:false
          }
        }).then(({data}) => {
            this.getDataFromServer();
